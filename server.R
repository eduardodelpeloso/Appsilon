...
shinyServer(function(input, output, session) {
  ...
  observeEvent(
    shipTypeStr(),
    update_dropdown_input(session, "ship_name-dropdown", sort(unique(ships_split[[shipTypeStr()]]["SHIPNAME"])[["SHIPNAME"]]),
  ...
  observeEvent(
    ...
    output$my_map <-renderLeaflet({
      ...
      leaflet() %>%
        ...
        addCircleMarkers(lat = result_ship_lat_lon[[2]][3],
                         lng = result_ship_lat_lon[[2]][1],
                         label = "Begin",
                         labelOptions = labelOptions(noHide = T) ) %>%
        addCircleMarkers(lat = result_ship_lat_lon[[2]][4],
